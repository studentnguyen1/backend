...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, CustomAuthenticationEntryPoint caep) throws Exception {
        http
                ...
                .authorizeHttpRequests(
                        authz -> authz
                                .requestMatchers("/", "/login", "/v3/api-docs/**", "/users",
                                        "/swagger-ui/**",
                                        "/swagger-ui.html")
                                .permitAll()
                ...
                .oauth2ResourceServer((oauth2) -> oauth2.jwt(Customizer.withDefaults()).authenticationEntryPoint(caep))
    ...
    // lay ra phan key de tao signature
    @Bean
    public JwtEncoder jwtEncoder() {
        return new NimbusJwtEncoder(new ImmutableSecret<>(getSecretKey()));
    }

    // chuyen key thanh Key() de tao token
    private SecretKey getSecretKey() {
        byte[] keyBytes = Base64.from(jwtKey).decode();
        return new SecretKeySpec(keyBytes, 0, keyBytes.length, SecurityUtil.JWT_ALGORITHM.getName());
    }

    // giai ma token va xax thuc token
    @Bean
    ...
    // convert token sang spring context
    @Bean
